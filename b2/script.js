...
function isValidPosition(gridRow, gridCol) {
    for (let dr = -1; dr <= 1; dr++) {
        for (let dc = -1; dc <= 1; dc++) {
            const checkRow = gridRow + dr;
            const checkCol = gridCol + dc;
            if (isValidCoordinates(checkRow, checkCol)) {
...
function isValidCoordinates(checkRow, checkCol) {
    let validRow = checkRow >= 0 && checkRow < GRID_SIZE;
    let validCol = checkCol >= 0 && checkCol < GRID_SIZE;
    ...
    for (const ship of SHIP_DEFINITIONS) {
        ...
        while (!placed) {
            ...
            if (isValidPlacement(currentShape, startRow, startCol)) {
                placeCurrentShip(currentShape, startRow, startCol, ship)
                placed = true;
            }
        }
    }
...
function placeCurrentShip(currentShape, startRow, startCol, ship) {
